...
# sesión). Se ejecuta UNA SOLA VEZ.
#
...
param(
    ...
    [int]$Puerto = 8000,
    # 4 procesos en paralelo: aprovecha el CPU sin agotar el límite de
...
if (-not (Test-Path $carpetaBackend)) {
    throw "No se encontró $carpetaBackend. Cloná el repositorio ahí primero, o ajustá -RaizProyecto."
}
...
if (-not (Test-Path $venv)) {
    python -m venv $venv
    if ($LASTEXITCODE -ne 0) { throw "Falló la creación del entorno. ¿Está Python 3.11 instalado y en el PATH?" }
}
...
Write-Host "[3/4] Verificando la configuración..." -ForegroundColor Cyan
...
if (-not (Test-Path $envFile)) {
    ...
    Write-Host "      IMPORTANTE: editá $envFile con la contraseña real antes de seguir." -ForegroundColor Yellow
}

Write-Host "[4/4] Registrando el arranque automático..." -ForegroundColor Cyan

# Se usa el Programador de tareas (nativo de Windows) en vez de un servicio
# clásico: no requiere instalar nada extra, arranca sin que nadie inicie
# sesión y se reintenta solo si el proceso se cae.
...
# SYSTEM: arranca con el equipo, sin depender de que un usuario inicie sesión.
...
try {
    ...
    Write-Host "`n  El backend todavía no responde." -ForegroundColor Yellow
    Write-Host "  Revisá el log: C:\AgroFresh\logs\backend.log" -ForegroundColor Yellow
}
