...
import { crearCarpeta, eliminar, listar, mover, renombrar, subirArchivos, urlDescarga } from '@/features/storage'
    ...
    <div>
      ...
      <Card>
        ...
        {entradas === null ? (
          ...
          <p className={styles.estado}>Esta carpeta está vacía.</p>
        ) : (
              ...
              <tbody>
                {entradas.map((e) => (
                  <tr
                    key={e.ruta}
                    draggable
                    onDragStart={(ev) => ev.dataTransfer.setData(TIPO_MOVER, e.ruta)}
                    onDragOver={(ev) => {
                      if (e.tipo === 'carpeta') {
                        ev.preventDefault()
                        setCarpetaSobrevolada(e.ruta)
                      }
                    }}
                    onDragLeave={() => setCarpetaSobrevolada((c) => (c === e.ruta ? null : c))}
                    onDrop={(ev) => e.tipo === 'carpeta' && onDropCarpeta(ev, e)}
                    className={cn(carpetaSobrevolada === e.ruta && styles.filaSobrevolada)}
                  >
                    <td className={styles.nombre}>
                      {e.tipo === 'carpeta' ? (
                        <button type="button" className={styles.nombreCarpeta} onClick={() => setRutaActual(e.ruta)}>
                    ...
                    <td className={styles.mono}>{formatDateTimeCL(e.modificado)}</td>
                    <td className={styles.acciones}>
                      {e.tipo === 'archivo' && (
                        <a className={styles.boton} href={urlDescarga(e.ruta)} target="_blank" rel="noreferrer">
                          Descargar
                        </a>
                      )}
                      <button className={styles.boton} onClick={() => renombrarEntrada(e)}>
                        Renombrar
                      </button>
                      <button className={styles.botonEliminar} onClick={() => eliminarEntrada(e)}>
                        Eliminar
                      </button>
                    </td>
                  </tr>
        ...
        <p className={styles.ayuda}>Puedes arrastrar un archivo o carpeta hacia otra carpeta de la lista para moverlo, o hacia las migas de arriba.</p>
      </Card>
    </div>
  )
}
