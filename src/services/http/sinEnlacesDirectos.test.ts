...
/** No llaman al backend: le pasan la URL al iframe, y ese sí manda el token
 * (ver la función `autorizacion` en public/modules/trace.html y el fetch
 * equivalente en public/modules/converter.html). */
const PASAN_LA_URL_AL_IFRAME = [
...
describe('llamadas al backend', () => {
  it('nadie arma URLs del backend fuera de la capa HTTP', () => {
    expect(
      revisar(/VITE_API_BASE_URL/, [CAPA_HTTP, ...PASAN_LA_URL_AL_IFRAME, TIPOS]),
